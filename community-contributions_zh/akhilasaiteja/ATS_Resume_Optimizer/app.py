...
# 【注】Hard technical tools — exact match only, never semantically substituted
EXACT_MATCH_ONLY = {
    # 【注】Languages
...
def _parse_date(date_str, use_gpt_fallback=True):
    ...
    # 【注】GPT fallback for unusual formats
    if use_gpt_fallback:
        try:
...
def _extract_experience_entries(resume_text):
    ...
    # 【注】Section headers that terminate the experience context window
    ...
    # 【注】Pass 1 — find all section-break line indices
    section_break_indices = set()
    for i, line in enumerate(lines):
        if _SECTION_BREAK_HEADERS.match(line):
            section_break_indices.add(i)

    # 【注】Pass 2 — collect indices of all lines that contain a date range
    # 【注】(excluding education context and lines inside non-experience sections)
    date_line_indices = []
    for i, line in enumerate(lines):
    ...
    for i in date_line_indices:
        ...
        for match in DATE_RANGE_RE.finditer(line):
            ...
            # 【注】Walk backwards to find the start boundary of this role.
            # 【注】Stop at: an empty line, a section header, or another date range line.
            start_boundary = i
            for j in range(i - 1, max(-1, i - 15), -1):
                candidate = lines[j].strip()
                # 【注】Empty line = role boundary
                if not candidate:
                    break
                # 【注】All-caps section header = role boundary
                if candidate.isupper() and len(candidate) < 50:
                    break
                # 【注】Another date range line = boundary of a different role
                if j in date_line_set and j != i:
                    break
                start_boundary = j

            # 【注】Walk forwards to find the end boundary of this role.
            # 【注】Hard stop: never cross a section-break header line.
            # 【注】Also stop at: next date range line, or empty line before a header/date.
            end_boundary = i
            for k in range(i + 1, min(len(lines), i + 20)):
                # 【注】Hard stop at any section-break header
                if k in section_break_indices:
                    break
                candidate = lines[k].strip()
                # 【注】Stop at any other date range line (next role)
                if k in date_line_set:
                    break
                # 【注】Stop at empty line followed by a section break or date
                if not candidate:
    ...
    # 【注】Deduplicate only on identical (start_label, end_label) pairs
    seen = set()
    unique_entries = []
    for e in entries:
...
def calculate_relevant_experience(resume_text, target_role, jd_text):
    ...
    # 【注】Build prompt for GPT to classify all jobs in one call
    jobs_block = '\n\n'.join([
...
{jobs_block}"""
    ...
    try:
        ...
        raw = resp.choices[0].message.content.strip()
        # 【注】Strip markdown code fences if present
        if '```' in raw:
            raw = re.sub(r'```(?:json)?', '', raw).strip().strip('`')
        parsed = json.loads(raw)
        # 【注】GPT might return {"jobs": [...]} or just [...]
        if isinstance(parsed, dict):
            classifications = next(iter(parsed.values()))
        else:
            classifications = parsed
    except Exception as e:
        # 【注】Fallback: treat all as partial
        classifications = [
            {
                'job_number': i + 1,
                'job_title': 'Unknown',
...
def calculate_keyword_score(jd_keywords, resume_full_text):
    ...
    if soft_skill_candidates:
        # 【注】One GPT call for all soft skill candidates
        batch_prompt = (
...
def deduplicate_skills(resume_text):
    ...
    # 【注】Find SKILLS header and the next all-caps header after it
    skills_start = None
    ...
    for i, line in enumerate(lines):
        ...
        if skills_start is None:
            ...
            # 【注】Next all-caps header (at least 3 chars, not a bullet)
    ...
    for i in range(skills_start + 1, skills_end):
        ...
        if not stripped.startswith(('•', '-', '*', '–', '▪')):
            continue
        # 【注】Extract bullet char + leading whitespace
        bullet_char = stripped[0]
        content = stripped[1:].strip()
        # 【注】Split on commas, deduplicate case-insensitively
...
def validate_no_fabrication(original_text, optimized_text, allowed_extras=None):
    ...
    def _current_section(up_to_idx):
        ...
        for j in range(up_to_idx + 1):
            ...
            if s in ('SKILLS', 'TECHNICAL SKILLS', 'SKILLS & TOOLS'):
                ...
                # 【注】Any other all-caps header resets to 'other'
    ...
    for i, line in enumerate(lines):
        ...
        if section == 'skills':
            # 【注】Remove individual comma-separated tokens that are entirely fabricated
            parts = [p.strip() for p in content.split(',') if p.strip()]
            kept = []
            for part in parts:
            ...
            # 【注】50% word-overlap check against original text
            shared = bullet_words & original_words
            ratio = len(shared) / len(bullet_words)

            if ratio < 0.0:
                # 【注】Revert to the nearest original bullet (highest word overlap)
                best_match = None
                best_overlap = -1
...
def clean_jd_keywords(keywords):
    ...
    for kw in keywords:
        ...
        if not kw_stripped:
            continue
        # 【注】Skip phrases longer than 4 words — too specific to be meaningful ATS keywords
        if len(kw_stripped.split()) > 4:
            continue
        # 【注】Skip single characters
        if len(kw_stripped) < 2:
            continue
        # 【注】Skip if matches clear noise pattern
        if any(re.search(p, kw_stripped.lower()) for p in NOISE_PATTERNS):
            continue
        cleaned.append(kw_stripped)

    # 【注】Deduplicate case-insensitively
    seen = set()
    final = []
    for kw in cleaned:
...
# 【注】── Experience bullet safety check ───────────────────────────────────────────
...
"""
    ...
    # 【注】Parse JSON — strip markdown fences, then repair control characters
    try:
        ...
        try:
            ...
            # 【注】GPT embedded literal newlines/tabs inside JSON string values — repair and retry
    ...
    # 【注】── Step 2: Post-processing ────────────────────────────────────────────────

    # 【注】2a. Deduplicate skills
    optimized_text = deduplicate_skills(result.get('optimized_resume', ''))

    # 【注】── Collect JD keywords from GPT JSON output ───────────────────────────────
    ...
    # 【注】Combined ATS score: keywords 50%, experience 30%, education 20%
    if required_years:
    ...
    # 【注】── Step 4: Accurate changes_made (line diff original vs final) ────────────
    return {
...
def should_regenerate(user_message):
    ...
    # 【注】Questions never trigger regeneration
    if msg.endswith('?'):
        return False
    if any(msg.startswith(q) for q in QUESTION_WORDS):
        return False

    # 【注】"I have/use/know X" without an explicit action word → observation, not request
    ...
    # 【注】Formatting/download requests — never touch resume content
    non_resume_triggers = [
        'download',
        'pdf format',
        'docx format',
    ...
    # 【注】Trigger only on explicit action words
...
def chat_with_resume(user_message, history, context):
    ...
    if 'UPDATED_RESUME:' in reply:
        ...
        # 【注】Recalculate keyword score with the same list used for initial scoring
...
def process_resume(resume_file, jd_text_input, run_count):
    ...
    # 【注】Build warm opening assistant message for chat
    missing = result['keywords_missing']
...
""")
        ...
        def _handle_chat(user_msg, history, context):
            ...
            # 【注】Yield 1: show user message immediately with typing indicator, clear input
            optimistic = history + [
            ...
            # 【注】Yield 2: full response after GPT call
