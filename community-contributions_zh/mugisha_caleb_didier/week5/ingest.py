"""摄取管道：将 KB 文档 + 摘要分块，嵌入到 ChromaDB 中。

首先运行generate_summaries.py，然后： uv run ingest.py"""
...
"""def make_messages(文档):
    返回[
        {“角色”：“用户”，“内容”：make_prompt（文档）}，
    ]


@重试（等待=等待）
def process_document(文档):
    消息= make_messages（文档）
    响应=完成（模型=模型，消息=消息，response_format=块）
    回复 = response.choices[0].message.content
    doc_as_chunks = Chunks.model_validate_json(reply).chunks
    返回 [chunk.as_result(document) for doc_as_chunks 中的块]


def create_chunks(文档):
    并行的“LLM-chunk 文档”。"""
    chunks = []
