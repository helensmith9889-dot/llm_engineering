...
class CourseSearcher:
    def __init__(self):
        ...
        # 为 OpenRouter 初始化 OpenAI 客户端
        or_key = os.environ.get("OPENROUTER_API_KEY")
        if not or_key:
            # 回退检查 OPENAI_API_KEY 是否以 sk-or-v1- 开头（有时用户在那里设置）
    ...
    def answer_question(self, query, top_k=5, history=None):
        ...
        try:
            # 我们必须指定 max_tokens 以避免超过低信用余额检查
            # google/gemini-2.5-flash 性价比极高且功能强大
